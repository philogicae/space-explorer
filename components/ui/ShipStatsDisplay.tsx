...
export function ShipStatsTracker({ shipRef }: { shipRef: React.RefObject<Group> }) {
  ...
  useFrame(() => {
    ...
    const roundedPosition = {
      x: Math.round(currentPosition.x / 50) * 50,
      y: Math.round(currentPosition.y / 50) * 50,
      z: Math.round(currentPosition.z / 50) * 50
    }
    
    
    const positionChanged = 
      Math.abs(roundedPosition.x - lastStablePosition.current.x) >= 50 ||
      Math.abs(roundedPosition.y - lastStablePosition.current.y) >= 50 ||
      Math.abs(roundedPosition.z - lastStablePosition.current.z) >= 50
  ...
  function calculateNearestCelestialBody(pos: {x: number, y: number, z: number}) {
    ...
    const bodies = [
      ...
      // Asteroid Belt is a ring between 2.2-3.2 AU
...
export default function ShipStatsDisplay() {
  ...
  return (
    <div className="absolute bottom-4 left-4 bg-black bg-opacity-70 text-white p-3 rounded font-mono text-sm z-10">
